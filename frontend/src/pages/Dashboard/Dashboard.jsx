...
import { ArrowRight, BrainCircuit, CheckCircle2, ClipboardCheck, FileCheck2, FolderKanban, GraduationCap, LayoutDashboard, Menu, Network, Plus, ScrollText, Search, Settings, Share2, ShieldCheck, Sparkles, Target, Upload, Users, X } from "lucide-react";
...
export default function Dashboard() {
  ...
  return <main className="main-app">
    ...
    <section className="app-content"><header className="app-header"><button className="mobile-menu" onClick={() => setOpen(true)}><Menu size={21} /></button><div className="app-search"><Search size={17} /><input placeholder={t("nav.searchPlaceholder", "Search your profile...")} /></div><div className="header-actions"><button className="share-chiplike" onClick={() => setShowShare(true)} disabled={!profile.username}><Share2 size={13} /> {t("nav.share", "Share")}</button><NotificationBell /><LanguageSwitcher /><ThemeSwitcher /><div className="user-chip"><span>ST</span><div><b>{t("nav.studentLabel", "Student")}</b><small>{t("nav.studentProfileShort", "Student profile")}</small></div></div></div></header>
      <div className="app-body profile-body"><div className="app-hero-row"><div><div className="mini-kicker"><span /> {t("app.kicker", "YOUR UPNEX PROFILE")}</div><h1>{t("app.heroTitleA", "Turn your work into ")}<em>{t("app.heroTitleB", "direction.")}</em></h1><p>{t("app.heroSub", "One verified record for your academics, skills and next opportunity.")}</p></div><div className="readiness-pill"><strong>{profile.readiness}</strong><span>{t("app.readinessFull", "% readiness")}</span></div></div>
        {message && <div className="profile-message"><CheckCircle2 size={16} /> {message}</div>}
        <section className="workflow-grid"><WorkflowStep number="01" icon={ShieldCheck} title={t("app.verify", "Verify")} text={t("app.verifySub", "Upload marksheets and certificates to a secure, signed-link vault.")} active={step === "vault"} onClick={() => setStep("vault")} /><WorkflowStep number="02" icon={Target} title={t("app.understand", "Understand")} text={t("app.understandSub", "Turn marks into clear academic and skill signals.")} active={step === "analysis"} onClick={() => setStep("analysis")} /><WorkflowStep number="03" icon={BrainCircuit} title={t("app.navigate", "Navigate")} text={t("app.navigateSub", "Match your evidence to real career paths.")} active={step === "careers"} onClick={() => setStep("careers")} /><WorkflowStep number="04" icon={FolderKanban} title={t("app.share", "Share")} text={t("app.shareSub", "Publish one living profile that recruiters can trust.")} active={step === "portfolio"} onClick={() => setStep("portfolio")} /></section>
